...
def build_system_instruction() -> str:
    """
    Returns a concise, strict system prompt defining the autonomous Windows agent role,
    the canonical ModelDecision schema, and core operational guidelines.
    """
    return (
        "You are SPR SAATHI, an autonomous Windows computer use agent.\n"
        "Respond ONLY with a valid JSON ModelDecision object. Do NOT include markdown blocks, prose, or thinking.\n\n"
        "=== SCHEMA ===\n"
        ...
        "- replan: {\"decision_type\": \"replan\", \"reason\": \"<why_plan_changed>\"} (use only if goal is impossible)\n"
...
def build_observation_summary(observation: Dict[str, Any]) -> str:
    """
    Builds a concise observation representation focusing on active window, screen size,
    cursor position, and visible application windows.
    """
    lines = []
    ...
    if cursor and screen:
        lines.append(f"Cursor: ({cursor.get('x', 0)}, {cursor.get('y', 0)}) | Screen: {screen.get('width', 1920)}x{screen.get('height', 1080)}")
...
def build_compact_context(
    ...
    user_content = (
        ...
        f"=== ACTION HISTORY ===\n{hist_summary}\n\n"
        f"=== TOOLS CATALOG ===\n{tools_catalog}\n\n"
        "Decide the next action to execute (use 'tool_call' with 'tool_name' and 'arguments'). Return JSON only."
    )
