...
class LocalModelProvider(BaseModelProvider):
    async def generate(self, prompt: str, system_instruction: Optional[str] = None) -> ModelResponse:
        # Simple completion placeholder
        return ModelResponse(
            text=f"[Local Model '{self.model_name}'] Mock completion for: {prompt[:50]}...",
            raw_response={"provider": "local", "model": self.model_name}
        )

    async def generate_with_tools(self, prompt: str, tools: List[Dict[str, Any]], system_instruction: Optional[str] = None) -> ModelResponse:
        # Mock structured tool call sequence based on prompt keywords
        text = f"[Local Model '{self.model_name}'] Mock reasoning for: {prompt}"
        tool_calls = []
        ...
        return ModelResponse(
            ...
            raw_response={"provider": "local", "model": self.model_name, "mocked": True}
        )
